...
FormulaRefRoot.ParseRange = function (range: string): {
  cr1: CrPartsWithCoord;
  cr2: CrPartsWithCoord;
} {
...
FormulaRefRoot.ReplaceFormulaCoords = function (formula: string, movedto: MovedToMap): string {
  const scf = SocialCalc.Formula;
  const tokentype = scf.TokenType;
  const token_op = tokentype.op;
  const token_string = tokentype.string;
