...
function tokenize(html: string): Array<{
...
/**
 * Install the extensions. Safe to call multiple times per Vitest worker; we
    ...
    const enhanceElement = (el: any) => {
      ...
      const styleProxy = new Proxy(base, {
        ...
        get(target, prop) {
          ...
          if (typeof prop === "string" && /^(padding|margin)(Top|Right|Bottom|Left)$/.test(prop)) {
            return "0px";
          }
          return v;
        },
