...
test("filldown preserves HTML formula quotes across rows (#493)", async () => {
  ...
  expect(sheet.cells.A6.formula).toBe(
    '=IF(B6=TODAY(),"<span style=""background-color:rgb(81,184,72);color:rgb(81,184,72)"">_______</span>","")'.slice(
      1,
    ),
  );
});
...
test("fillright preserves HTML formula quotes across columns (#493)", async () => {
  ...
  expect(sheet.cells.B4.formula).toBe(
    '=IF(C4=TODAY(),"<span style=""background-color:rgb(81,184,72);color:rgb(81,184,72)"">_______</span>","")'.slice(
      1,
    ),
  );
  expect(sheet.cells.C4.formula).toBe(
    '=IF(D4=TODAY(),"<span style=""background-color:rgb(81,184,72);color:rgb(81,184,72)"">_______</span>","")'.slice(
      1,
    ),
  );
});
