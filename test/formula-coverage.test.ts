...
test("parser handles numbers, strings, operators, comparisons, and unary ops", async () => {
  ...
  const nums = scientific.filter((t: any) => t.type === SC.Formula.TokenType.num);
  ...
  expect(badExp.some((t: any) => t.type === SC.Formula.TokenType.error)).toBe(true);

  const noClose = SC.Formula.ParseFormulaIntoTokens('"oops');
  expect(noClose.some((t: any) => t.type === SC.Formula.TokenType.error)).toBe(true);

  const twoops = SC.Formula.ParseFormulaIntoTokens("*+");
  expect(twoops.some((t: any) => t.type === SC.Formula.TokenType.error)).toBe(true);

  const unknownChar = SC.Formula.ParseFormulaIntoTokens("@");
  expect(unknownChar.some((t: any) => t.type === SC.Formula.TokenType.error)).toBe(true);

  const badSpecial = SC.Formula.ParseFormulaIntoTokens("#NOEND");
  expect(badSpecial.some((t: any) => t.type === SC.Formula.TokenType.error)).toBe(true);

  const missingOpen = SC.Formula.ConvertInfixToPolish(SC.Formula.ParseFormulaIntoTokens("1,2"));
  ...
  const stray = SC.Formula.ConvertInfixToPolish(SC.Formula.ParseFormulaIntoTokens("1+2)"));
  expect(typeof stray).toBe("string");

  const errorBubbled = SC.Formula.ConvertInfixToPolish(SC.Formula.ParseFormulaIntoTokens("*+"));
...
test("parser/evaluator error and helper branches", async () => {
  ...
  const polishErr = SC.Formula.ConvertInfixToPolish(SC.Formula.ParseFormulaIntoTokens("1+"));
  ...
  const blankNum = SC.Formula.OperandAsNumber(sheet, [{ type: "b", value: "" }]);
  expect(blankNum.value).toBe(0);
  expect(blankNum.type).toBe("n");

  const blankTxt = SC.Formula.OperandAsText(sheet, [{ type: "b", value: "" }]);
  expect(blankTxt.value).toBe("");

  const numTxt = SC.Formula.OperandAsText(sheet, [{ type: "n", value: 5 }]);
  expect(numTxt.type).toBe("t");

  const errorOp = SC.Formula.OperandAsNumber(sheet, [{ type: "e#VALUE!", value: "boom" }]);
  expect(errorOp.value).toBe(0);
  expect(errorOp.type.charAt(0)).toBe("e");

  const errorTxt = SC.Formula.OperandAsText(sheet, [{ type: "e#VALUE!", value: "boom" }]);
  ...
  const coord = SC.Formula.OperandAsCoord(sheet, [{ type: "coord", value: "A1" }]);
  expect(coord.value).toBe("A1");
  const notCoord = SC.Formula.OperandAsCoord(sheet, [{ type: "n", value: 5 }]);
  expect(notCoord.type).toBe("e#REF!");

  const rng = SC.Formula.OperandAsRange(sheet, [{ type: "range", value: "A1|A2|" }]);
  expect(rng.type).toBe("range");

  const topEmpty = SC.Formula.TopOfStackValueAndType(sheet, []);
  expect(topEmpty.error).toBeTruthy();

  const nameResolved = SC.Formula.TopOfStackValueAndType(sheet, [{ type: "name", value: "R" }]);
  ...
  expect(SC.Formula.CheckForErrorValue(op, { type: "e#NUM!", value: "x" })).toBe(true);
...
test("LookupResultType quirks and type tables", async () => {
  ...
  expect(SC.Formula.LookupResultType("e#VALUE!", "n", tlu.plus)).toBe("e#VALUE!");
  expect(SC.Formula.LookupResultType("n", "n", tlu.unaryminus)).toBe("n");
  expect(SC.Formula.LookupResultType("t", "t", tlu.unaryminus)).toBe("e#VALUE!");
...
test("evaluate_parsed_formula: missing operand scenarios", async () => {
  ...
  await scheduleCommands(SC, sheet, ["set A1 value n 5", "name define MYONE A1"]);
  await recalcSheet(SC, sheet);
...
test("TopOfStackValueAndType handles name → range resolution", async () => {
  ...
  await recalcSheet(SC, sheet);
  const coordResult = SC.Formula.TopOfStackValueAndType(sheet, [{ type: "name", value: "JUSTA" }]);
...
test("OperandAsSheetName via cell reference (sheet name in A1)", async () => {
  const SC = await loadSocialCalc();
  resetFormulaGlobals(SC);
  SC.Formula.AddSheetToCache("OTHERSHEET", "version:1.5\ncell:A1:v:42\nsheet:c:1:r:1\n", false);

  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set A1 text t OTHERSHEET", "set A2 formula SUM(A1!A1:A1)"]);
  await recalcSheet(SC, sheet);
  expect(sheet.GetAssuredCell("A2").datavalue).toBe(42);
});
...
test("Parser coverage: specialvalue, names+coords, operator edges", async () => {
  ...
  expect(bad.some((t: any) => t.type === SC.Formula.TokenType.error)).toBe(true);
});
...
test("parser: unary minus vs binary minus (L376/L377)", async () => {
  ...
  const pctMinus = afterPct.filter((t: any) => t.text === "-" && t.opcode !== "M");
...
test("parser: '()' null arg list allowed without error (L385)", async () => {
  ...
  // No errors
  expect(tok.some((t: any) => t.type === SC.Formula.TokenType.error)).toBe(false);
  ...
  expect(err.filter((t: any) => t.type === SC.Formula.TokenType.error).length).toBe(0);

  // ")+" with no opener should yield an error somewhere downstream.
  // But the tokenizer might not flag it; ConvertInfixToPolish will.
  const notNull = SC.Formula.ConvertInfixToPolish(SC.Formula.ParseFormulaIntoTokens("1+)2"));
...
test("parser: space is ignored (L418)", async () => {
  ...
  expect(tok.filter((t: any) => t.type === SC.Formula.TokenType.space).length).toBe(0);
});
...
test("EvaluatePolish: revpolish is a string (parse error) returns e#VALUE! (L630)", async () => {
  ...
  const r = SC.Formula.EvaluatePolish([fakeToken], "an error string", sheet, false);
...
test("EvaluatePolish: numeric comparison ops all 6 (L751-L758)", async () => {
  ...
  const sheet = new SC.Sheet();
  const ev = (s: string) =>
    SC.Formula.evaluate_parsed_formula(SC.Formula.ParseFormulaIntoTokens(s), sheet, false);
...
test("LookupResultType: plain hit returns result type (L951-L958)", async () => {
  ...
  const r = SC.Formula.LookupResultType("n", "n", SC.Formula.TypeLookupTable.plus);
  expect(r).toBe("n");

  // "n%" plus "n%" → "n%".
  const r2 = SC.Formula.LookupResultType("n%", "n%", SC.Formula.TypeLookupTable.plus);
  expect(r2).toBe("n%");
});
...
test("LookupResultType: result '1' returns type1, result '2' returns type2 (L956/L957)", async () => {
  ...
  const r = SC.Formula.LookupResultType("n", "n", SC.Formula.TypeLookupTable.unaryminus);
  expect(r).toBe("n");

  // plus['n']['e*:2'] — when type2 is e*, result 2 means type2 returned.
  const r2 = SC.Formula.LookupResultType("n", "e#DIV/0!", SC.Formula.TypeLookupTable.plus);
...
test("LookupResultType: uses major-type (*) fallback when type2 exact not found (L960-L967)", async () => {
  ...
  const r = SC.Formula.LookupResultType("n", "nfunky", SC.Formula.TypeLookupTable.plus);
  expect(r).toBe("n");

  // And for "2" in the fallback: plus['t*'] fallback: {'e*':2}. When type1=t*, type2=e*, result is type2.
  const r2 = SC.Formula.LookupResultType("t", "e#DIV/0!", SC.Formula.TypeLookupTable.plus);
  expect(r2).toBe("e#DIV/0!");
});
...
test("LookupResultType: type1 not in table → uses type1.charAt(0)+'*' (L946)", async () => {
  ...
  const r = SC.Formula.LookupResultType("nfunky", "n", SC.Formula.TypeLookupTable.plus);
...
test("LookupResultType: truly missing → e#VALUE! with 'missing' text (L948)", async () => {
  ...
  const r = SC.Formula.LookupResultType("zfunky", "z", SC.Formula.TypeLookupTable.plus);
...
test("LookupResultType: pair not found in either table → e#VALUE! (L969)", async () => {
  ...
  const r = SC.Formula.LookupResultType("n", "z", SC.Formula.TypeLookupTable.plus);
...
test("OperandAsType: name resolves then checks type (L1216)", async () => {
  ...
  resetFormulaGlobals(SC);
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set A1 value n 1", "name define JUSTA A1"]);
  await recalcSheet(SC, sheet);
...
test("OperandsAsCoordOnSheet: error type propagates (L1275-L1277)", async () => {
  const SC = await loadSocialCalc();
  resetFormulaGlobals(SC);
  SC.Formula.AddSheetToCache("OTHERZ", "version:1.5\ncell:A1:v:1\nsheet:c:1:r:1\n", false);
  const sheet = new SC.Sheet();
...
test("OperandsAsCoordOnSheet: non-coord/range/error → e#REF! fallback (L1278-L1282)", async () => {
  const SC = await loadSocialCalc();
  resetFormulaGlobals(SC);
  SC.Formula.AddSheetToCache("OTHERW", "version:1.5\ncell:A1:v:1\nsheet:c:1:r:1\n", false);
  const sheet = new SC.Sheet();
  const op = [
    { type: "t", value: "OTHERW" },
...
test("OperandAsSheetName: name resolves to string type (L1362-L1369)", async () => {
  const SC = await loadSocialCalc();
  resetFormulaGlobals(SC);
  SC.Formula.AddSheetToCache("NAMED", "version:1.5\nsheet:c:1:r:1\n", false);
  const sheet = new SC.Sheet();
  // Define a name that points to a text value via cell
  await scheduleCommands(SC, sheet, ["set A1 text t NAMED", "name define SN A1"]);
  await recalcSheet(SC, sheet);
...
test("EvaluatePolish: a name that resolves becomes final value (L865-L869)", async () => {
  ...
  resetFormulaGlobals(SC);
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set A1 value n 11", "name define ALPHA A1"]);
  await recalcSheet(SC, sheet);
...
test("ConvertInfixToPolish: stray ')' without '(' → error (L528-L531)", async () => {
  const SC = await loadSocialCalc();
  const result = SC.Formula.ConvertInfixToPolish(SC.Formula.ParseFormulaIntoTokens("1+2)"));
  expect(typeof result).toBe("string");
});

test("ConvertInfixToPolish: unmatched '(' → error (L569-L571)", async () => {
  const SC = await loadSocialCalc();
  const result = SC.Formula.ConvertInfixToPolish(SC.Formula.ParseFormulaIntoTokens("SUM(1+2"));
  expect(typeof result).toBe("string");
});
...
test("LookupName: circular reference detected (L1418-L1422)", async () => {
  ...
  resetFormulaGlobals(SC);
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["name define LOOP =LOOP+1"]);
...
test("OperandValueAndType: name operand follows name → coord (L1123-L1125)", async () => {
  ...
  resetFormulaGlobals(SC);
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set A1 value n 100", "name define ONE A1"]);
  await recalcSheet(SC, sheet);
...
test("LookupName: simple coord name resolution", async () => {
  ...
  resetFormulaGlobals(SC);
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set B3 value n 42", "name define X B3"]);
  await recalcSheet(SC, sheet);
...
test("LookupName: case insensitive lookup", async () => {
  ...
  resetFormulaGlobals(SC);
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set A1 value n 7", "name define HELLO A1"]);
  await recalcSheet(SC, sheet);
...
test("parser: alphanumeric terminated by all 4 class types (L281-L282)", async () => {
  ...
  // op (+)
  expect(SC.Formula.ParseFormulaIntoTokens("X1Y+1").map((t: any) => t.text)).toEqual([
    "X1Y",
    "+",
    "1",
  ]);
...
test("parser: coord terminated by all 4 class types (L259)", async () => {
  const SC = await loadSocialCalc();
  // op
  expect(SC.Formula.ParseFormulaIntoTokens("A1+2").map((t: any) => t.text)).toEqual([
    "A1",
    "+",
    "2",
  ]);
...
test("ConvertInfixToPolish: parens override precedence (L541-L542 '(' blocker)", async () => {
  ...
  // (1+2)*3 vs 1+2*3
  expect(
    SC.Formula.evaluate_parsed_formula(SC.Formula.ParseFormulaIntoTokens("(1+2)*3"), sheet, false)
      .value,
  ).toBe(9);
  expect(
    SC.Formula.evaluate_parsed_formula(SC.Formula.ParseFormulaIntoTokens("1+2*3"), sheet, false)
...
test("LookupResultType: plus with 'b' types (blank) — result '1'", async () => {
  ...
  const r = SC.Formula.LookupResultType("b", "b", SC.Formula.TypeLookupTable.plus);
  expect(r).toBe("n");
});

test("LookupResultType: oneargnumeric — raw result 'n'", async () => {
  const SC = await loadSocialCalc();
  const r = SC.Formula.LookupResultType("n", "n", SC.Formula.TypeLookupTable.oneargnumeric);
...
test("comparison: tostype='n', tostype2='t' routes via text branch (L768/L769)", async () => {
  ...
  await scheduleCommands(SC, sheet, ["set A1 value n% 0.5", 'set B1 formula A1<"z"']);
  await recalcSheet(SC, sheet);
