...
// Mirror the reset used in formula-functions-coverage.test.ts. The SocialCalc
// instance is shared within this Vitest file worker, so prior tests can leave
...
async function freshApp(SC: any, idSuffix: string): Promise<CtrlBundle> {
...
test("TriggerIoAction.Email with coord and text parameters", async () => {
  ...
  await scheduleCommands(SC, sheet, ["set B1 text t recipient@example.com"]);
  await recalcSheet(SC, sheet);
...
test("TriggerIoAction.AddAutocomplete wires jQuery UI autocomplete", async () => {
  ...
  await scheduleCommands(SC, sheet, ["set A1 text t alpha", "set A2 text t bravo"]);
  await recalcSheet(SC, sheet);
...
test("IoFunctions PANEL renders with SocialCalc._app=true and real control", async () => {
  ...
  try {
    ...
    SC.Formula.IoFunctions("SPLASH", op3, [{ type: "range", value: "A1|B1|" }], sheet, "Z3");
...
test("LocalizeString + LocalizeSubstrings cover their code paths", async () => {
  ...
  expect(SC.LocalizeSubstrings("a %ssc!__test_localize_constant__! b")).toContain("ok-value");
});
...
// --------------------------------------------------------------------------
// TriggerIoAction.Email coverage. Vitest isolates test files, so stubs in the
// editor suites cannot replace this worker's implementation.
// --------------------------------------------------------------------------
...
test("TriggerIoAction.AddAutocomplete select + change callbacks execute", async () => {
  ...
  await scheduleCommands(SC, sheet, ["set A1 text t alpha", "set A2 text t beta"]);
  await recalcSheet(SC, sheet);
