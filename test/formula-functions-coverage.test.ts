...
test("Math1 TAN at cos=0 gives NUM error (via direct eval of tan near pi/2)", async () => {
  ...
  try {
    Math.cos = () => 0;
    Math.tan = () => NaN;
    SC.Formula.Math1Functions("TAN", operand, [{ type: "n", value: 1.5 }], new SC.Sheet());
...
test("CalculateFunction: STYLE via FillFunctionInfo and direct IoFunctions dispatch", async () => {
  ...
  const styleOp: any[] = [];
  SC.Formula.IoFunctions("STYLE", styleOp, [{ type: "t", value: "color:red" }], sheet, "A1");
...
test("IoFunctions BUTTON/TEXTBOX/IMAGEBUTTON/AUTOCOMPLETE direct invocation", async () => {
  ...
  let op: any[] = [];
  SC.Formula.IoFunctions("BUTTON", op, [{ type: "t", value: "Click" }], sheet, "B1");
  expect(op[0].value).toBe("Click");
  expect(op[0].type).toBe("tiBUTTON");

  // IMAGEBUTTON
  op = [];
  SC.Formula.IoFunctions("IMAGEBUTTON", op, [{ type: "t", value: "img.png" }], sheet, "B2");
  expect(op[0].type).toBe("tiIMAGEBUTTON");

  // TEXTBOX
  op = [];
  SC.Formula.IoFunctions("TEXTBOX", op, [{ type: "t", value: "Hello" }], sheet, "B3");
  ...
  // SUBMIT with arg
  op = [];
  SC.Formula.IoFunctions("SUBMIT", op, [{ type: "t", value: "Go" }], sheet, "B5");
...
test("IoFunctions CHECKBOX/RADIOBUTTON true/false paths", async () => {
  ...
  let op: any[] = [];
  SC.Formula.IoFunctions("CHECKBOX", op, [{ type: "t", value: "TRUE" }], sheet, "C1");
  expect(op[0].value).toBe(1);

  // CHECKBOX with text "false"
  op = [];
  SC.Formula.IoFunctions("CHECKBOX", op, [{ type: "t", value: "false" }], sheet, "C2");
  expect(op[0].value).toBe(0);

  // CHECKBOX with number 1
  op = [];
  SC.Formula.IoFunctions("CHECKBOX", op, [{ type: "n", value: 1 }], sheet, "C3");
  expect(op[0].value).toBe(1);

  // CHECKBOX with number 0
  op = [];
  SC.Formula.IoFunctions("CHECKBOX", op, [{ type: "n", value: 0 }], sheet, "C4");
  expect(op[0].value).toBe(0);
...
test("IoFunctions COPYVALUE/COMMAND read trigger cell value", async () => {
  ...
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set A1 value n 42", "set A2 text t images/pic.png"]);
  await recalcSheet(SC, sheet);
...
test("IoFunctions with error arg propagates", async () => {
  ...
  const op: any[] = [];
  SC.Formula.IoFunctions("BUTTON", op, [{ type: "e#VALUE!", value: "boom" }], sheet, "Z1");
  expect(op[0].type.charAt(0)).toBe("e");
});
...
test("IoFunctions PANEL/SPLASH direct invocation without app", async () => {
  ...
  let op: any[] = [];
  SC.Formula.IoFunctions("SPLASH", op, [{ type: "range", value: "A1|A2|" }], sheet, "Z1");
  expect(op[0].type).toBe("t");
...
test("IoFunctions: invalid type for argdef 1/2/3 (OperandAsNumber/Text failing)", async () => {
  ...
  const op: any[] = [];
  SC.Formula.IoFunctions("BUTTON", op, [{ type: "range", value: "A1|A2|" }], sheet, "Z1");
  expect(op[0]).toBeDefined();
});
...
test("FieldToColnum: numeric fieldname out-of-range and wildcard text", async () => {
  ...
  resetFormulaGlobals(SC);
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set A1 text t name", "set B1 text t score"]);
  await recalcSheet(SC, sheet);
...
test("Math1Functions: LOG10/LN error text set", async () => {
  ...
  const operand1: any[] = [];
  SC.Formula.Math1Functions("LN", operand1, [{ type: "n", value: 0 }], sheet);
  expect(operand1[0].type).toBe("e#NUM!");

  const operand2: any[] = [];
  SC.Formula.Math1Functions("LOG10", operand2, [{ type: "n", value: -5 }], sheet);
...
test("StoreIoEventFormula: TimeTrigger with coord operand[0]", async () => {
  ...
  try {
    SC.Formula.StoreIoEventFormula("EMAILAT", "Z40", foperand, sheet, "TimeTrigger");
...
test("StoreIoEventFormula: TimeTrigger with string-wrapped numeric operand[0]", async () => {
  ...
  sheet.ScheduleSheetCommands = () => {};
  try {
    SC.Formula.StoreIoEventFormula("EMAILAT", "Z41", foperand, sheet, "TimeTrigger");
    expect(sheet.ioTimeTriggerList).toBeDefined();
  } finally {
    sheet.ScheduleSheetCommands = origSchedule;
  }
...
test("StoreIoEventFormula: TimeTrigger with range operand[0] (walks range)", async () => {
  ...
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set A1 value n 45500", "set A2 value n 45501"]);
  await recalcSheet(SC, sheet);
  sheet.ioTimeTriggerList = {};
  ...
  sheet.ScheduleSheetCommands = () => {};
  try {
    SC.Formula.StoreIoEventFormula("EMAILAT", "Z42", foperand, sheet, "TimeTrigger");
...
test("StoreIoEventFormula: EventTree with coord/range trigger (direct invocation)", async () => {
  ...
  SC.Formula.StoreIoEventFormula("COMMAND", "Z43", foperandCoord, sheet, "EventTree");
  ...
  SC.Formula.StoreIoEventFormula("COMMAND", "Z44", foperandRange, sheet, "EventTree");
  expect(sheet.ioEventTree.A1).toBeDefined();
...
test("LookupName: formula-defined name returning a non-range value", async () => {
  ...
  resetFormulaGlobals(SC);
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set A1 value n 5", "name define FOO =A1+10"]);
  await recalcSheet(SC, sheet);
...
test("StoreIoEventFormula: Input branch (TEXTBOX) via CalculateFunction", async () => {
  ...
  try {
    ...
    expect(SC.CurrentSpreadsheetControlObject.formDataViewer.formFields).toBeDefined();
  } finally {
...
test("LoadFormFields: exercises cell-walk loop via CurrentSpreadsheetControlObject", async () => {
  ...
  const formSheet = new SC.Sheet();
  await scheduleCommands(SC, formSheet, ["set B1 text t alpha", "set C1 text t beta"]);
  ...
  try {
    ...
    const ff = SC.CurrentSpreadsheetControlObject.formDataViewer.formFields;
