...
test("exportCSV applies number format rounding (#638)", async () => {
  ...
  await scheduleCommands(
    SC,
    sheet,
    ["set A1 value n 1.9859735", "set A1 nontextvalueformat #,##0", "recalc"],
    true,
    3000,
  );
  await recalcSheet(SC, sheet, 3000);
...
test("exportCSV formats date cells (#355)", async () => {
  ...
  await scheduleCommands(
    SC,
    sheet,
    ["set A1 value d 2010-01-01", "set A1 nontextvalueformat m/d/yyyy", "recalc"],
    true,
    3000,
  );
  await recalcSheet(SC, sheet, 3000);
...
test("division preserves currency valuetype (#577)", async () => {
  ...
  await scheduleCommands(
    SC,
    sheet,
    ["set A1 constant n$ 142.234 $142.234", "set C1 formula A1/2", "recalc"],
    true,
    3000,
  );
  await recalcSheet(SC, sheet, 3000);
