...
test("EditorGetStatuslineString handles many statuses", async () => {
  ...
  expect(editor.GetStatuslineString("calcorder", { count: 3, total: 10 }, params)).toContain("A1");
  expect(editor.GetStatuslineString("calcstep", { count: 6, total: 10 }, params)).toContain("A1");
  expect(editor.GetStatuslineString("calcloading", { sheetname: "other" }, params)).toContain(
    "other",
  );
  expect(
...
test("SpreadsheetControl: save / load round-trip", async () => {
  ...
  await scheduleCommands(SC, control.sheet, ["set A1 value n 100", "set B1 value n 200"]);
  await recalcSheet(SC, control.sheet);
...
test("SpreadsheetControl: movefrom/movepaste/moveinsert cycle", async () => {
  ...
  await scheduleCommands(SC, control.sheet, ["set A1 value n 5", "set A2 value n 10"]);
  await recalcSheet(SC, control.sheet);
  control.editor.MoveECell("A1");
...
test("TableEditor: EditorApplySetCommandsToRange", async () => {
  ...
  await scheduleCommands(SC, control.sheet, ["set A1 value n 1", "set A2 value n 2"]);
  await recalcSheet(SC, control.sheet);
  control.editor.MoveECell("A1");
  const p1 = waitEditor(control.editor);
...
test("SpreadsheetControl Comment + Names helpers operate", async () => {
  ...
  await scheduleCommands(SC, control.sheet, ["name define SUM1 A1:A3", "name desc SUM1 my_sum"]);
