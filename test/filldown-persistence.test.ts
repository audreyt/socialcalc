...
function installInteractiveRange2EditorMock(SC: SocialCalcWithFillEditor, range2: Range2Rect) {
...
test("fillright without editor.range2 increments from command range", async () => {
  ...
  installHeadlessEditorMock(SC);
  const sheet = new SC.Sheet();
  await scheduleCommands(SC, sheet, ["set A1 value n 10", "set B1 value n 20"]);
