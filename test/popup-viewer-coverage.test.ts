...
function wrapTimerGlobals() {
  const wrappedSetTimeout = function (handler: any, timeout?: number, ...args: any[]) {
...
test("Viewer: DoButtonCmd recalc + unknown + blur element", async () => {
  ...
  (btn as any).blur = () => {};
  SC.SpreadsheetViewerDoButtonCmd({ target: btn }, null, {
    element: btn,
    functionobj: { command: "recalc" },
  });
  SC.SpreadsheetViewerDoButtonCmd({ target: btn }, null, {
    element: btn,
    functionobj: { command: "unknowncmd" },
  });
  // Element without blur still runs through.
  SC.SpreadsheetViewerDoButtonCmd({ target: null }, null, {
    element: null,
    functionobj: { command: "recalc" },
  });
});
...
test("Viewer: DoRepeatingMacro + RepeatMacroCommand", async () => {
  ...
  try {
    SC.SpreadsheetViewerRepeatMacroCommand("repeatmacro", null, viewer.sheet, cmd1, false);
...
test("Viewer: DecodeSpreadsheetSave early-returns on malformed inputs", async () => {
  ...
  expect(viewer.DecodeSpreadsheetSave("MIME-Version: 1.0\nhello\n")).toEqual({});
  // Boundary present but no top boundary.
  const str1 = ["MIME-Version: 1.0", "Content-Type: multipart/mixed; boundary=XX", ""].join("\n");
