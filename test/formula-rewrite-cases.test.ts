...
function parseDirectCase(value: unknown, index: number): DirectCase {
  ...
  const { name, method, formula, args, expected, invariant } = value;
  ...
  if ("movedto" in args) {
    ...
    for (const [key, entry] of Object.entries(args.movedto)) {
      if (typeof entry !== "string") {
        throw new Error(`directCases[${index}].args.movedto.${key} must be a string`);
      }
      movedto[key] = entry;
    }
...
function parseCommandCase(value: unknown, index: number): CommandCase {
  ...
  if (!Array.isArray(commands) || !commands.every((line) => typeof line === "string")) {
  ...
  const cells: CommandExpectedCell[] = expectedCells.map((cell, cellIndex) => {
    if (!isRecord(cell)) {
      throw new Error(`commandCases[${index}].expectedCells[${cellIndex}] must be an object`);
    }
    const { coord, field, expected } = cell;
    if (typeof coord !== "string") {
      throw new Error(`commandCases[${index}].expectedCells[${cellIndex}].coord must be a string`);
    }
    if (typeof field !== "string") {
      throw new Error(`commandCases[${index}].expectedCells[${cellIndex}].field must be a string`);
    }
    if (typeof expected !== "string") {
      throw new Error(
...
function loadFixtures(): FormulaRewriteFixtures {
  const url = new URL("./fixtures/formula-rewrite-cases.json", import.meta.url);
...
function callDirect(
  SC: {
    ...
    ReplaceFormulaCoords: (formula: string, movedto: Record<string, string>) => string;
  },
  c: DirectCase,
): string {
  switch (c.method) {
    case "offset":
      return SC.OffsetFormulaCoords(c.formula, c.args.coloffset ?? 0, c.args.rowoffset ?? 0);
    case "adjust":
