...
test("loads the SocialCalc bundle through the VP test harness", async () => {
...
test("waitForStatus rejects when the expected status never arrives", async () => {
  const fakeSheet = { statuscallback: null } as unknown as { statuscallback: unknown };
  let error: unknown;
  try {
    await waitForStatus(fakeSheet, "never-fires", () => {}, 5);
  } catch (e) {
    error = e;
  }
